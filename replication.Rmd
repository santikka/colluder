---
title: 'Full Law Identification under Missing Data in the Categorical Colluder Model:
  ...
  for (j in 1:nrow(cases)) {
    if (cases$x[j] != "m") { # X observed
      ...
      if (cases$y[j] != "m") { # X and Y observed
        ftext <- paste0(
          ftext," + ", nxylevels[j], " * log(", prefix, "ry_", cases$x[j], "_1",   ")"
        )
        ftext <- paste0(
...
vectorize_logl <- function(loglfunction, paramnames, prefix = "p_") { # works also for gradient 
  vlogl <- function(param, nvec) {
    if (any(!is.finite(param))) return(NA)
    if (min(param) < 0 | max(param) > 1) return(NA)
    sum_p_x <- sum(param[substr(names(param), 1, 1 + nchar(prefix)) == paste0(prefix, "x")]) 
    if (sum_p_x >= 1) return(NA)
    for (k in 0:9) { # assuming max 10 classes
      sum_p_y <- sum(param[substr(names(param), 1, 3 + nchar(prefix)) == paste0(prefix, "y_", k)])
      if (sum_p_y >= 1) return(NA)
    }
...
vectorize_grad <- function(gradfunction, paramnames, prefix = "p_") { # works also for gradient 
  vlogl <- function(param, nvec) {
    if (any(!is.finite(param))) return(rep(NA, length(param)))
    if (min(param) < 0 | max(param) > 1) return(rep(NA, length(param)))
    sum_p_x <- sum(param[substr(names(param), 1, 1 + nchar(prefix)) == paste0(prefix, "x")]) 
    if (sum_p_x >= 1) return(rep(NA, length(param)))
    for (k in 0:9) { # assuming max 10 classes
      sum_p_y <- sum(param[substr(names(param), 1, 3 + nchar(prefix)) == paste0(prefix, "y_", k)])
      if (sum_p_y >= 1) return(rep(NA, length(param)))
    }
    paramlist <- vector("list", length(paramnames))
    names(paramlist) <- paramnames
    for (j in 1:length(param)) {
...
create_colluder_model <- function(xclasses = 2, yclasses = 2, prefix = "p_", 
  ...
  y_x_prob <- matrix(runif(xclasses * yclasses, minrunif, maxrunif), nrow = xclasses, ncol = yclasses)
  ...
  ry_x_rx_prob1 <- matrix(runif(xclasses, ryminrunif, rymaxrunif), nrow = xclasses, ncol = 2) 
  ...
  scm <- R6causal::SCM$new(
    ...
    uflist = list(
      ...
      urx = function(n) {return(runif(n))},
      ury = function(n) {return(runif(n))}
    ),
    vflist = list(
      x = function(ux) {
        return(
    ...
    rflist = list(
      x = function(urx) { as.numeric(urx < rx_prob1) },
...
repry22 <- data.frame(
  ...
  bias_min = paste0("$", formatC(ry_meanres22$bias_min, digits = 4, format = "f"), "$"),
  bias_max = paste0("$", formatC(ry_meanres22$bias_max, digits = 4, format = "f"), "$"),
...
repother22 <- data.frame(
  ...
  bias_min = paste0("$", formatC(other_meanres22$bias_min, digits = 4, format = "f"), "$"),
  bias_max = paste0("$", formatC(other_meanres22$bias_max, digits = 4, format = "f"), "$"),
...
repry44 <- data.frame(
  ...
  bias_min = paste0("$", formatC(ry_meanres44$bias_min, digits = 4, format = "f"), "$"),
  bias_max = paste0("$", formatC(ry_meanres44$bias_max, digits = 4, format = "f"), "$"),
...
repother44 <- data.frame(
  ...
  bias_min = paste0("$", formatC(other_meanres44$bias_min, digits = 4, format = "f"), "$"),
  bias_max = paste0("$", formatC(other_meanres44$bias_max, digits = 4, format = "f"), "$"),
  rmse_mean = formatC(other_meanres44$rmse_mean, digits = 4, format = "f"),
  rmse_max = formatC(other_meanres44$rmse_max, digits = 4, format = "f")
)
```

```{r graduates, cache=TRUE}
...
knitr::kable(
  ...
  align = "llll",
  linesep = "",
  col.names = c("Parameter", "Estimate (95\\% CI)", "Parameter", "Est. (95\\% CI)")
)|>
  kableExtra::kable_styling(table.envir = "center")
```